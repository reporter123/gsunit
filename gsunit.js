...
GsUnit.DOUBLE_EQUALITY_PREDICATE = function (var1, var2)
{
	return var1 == var2;
};

/* Predicate used for testing JavaScript === (i.e. equality including type) */
GsUnit.TRIPLE_EQUALITY_PREDICATE = function (var1, var2)
{
	return var1 === var2;
};

/* Predicate used for testing whether two obects' toStrings are equal */
GsUnit.TO_STRING_EQUALITY_PREDICATE = function (var1, var2)
{
...
/* @param Any object @return String - the type of the given object @private */
...
GsUnit.validateArguments = function (expectedNumberOfNonCommentArgs, args)
{
	if (!(args.length == expectedNumberOfNonCommentArgs) ||
		(args.length == expectedNumberOfNonCommentArgs + 1 && (typeof (args0) == 'string') || args0 == null)))
...
{
	return GsUnit.argumentsIncludeComments(expectedNumberOfNonCommentArgs, args) ? argsdesiredNonCommentArgIndex : args - 1;
}
...
/* All assertions ultimately go through this method. @private */
...
/* Checks that two values are equal (using ===) @param {String} comment optional, displayed in the case of failure @param {Value} expected the expected value @param {Value} actual the actual value @throws GsUnit.Failure if the values are not equal @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that the given boolean value is true. @param {String} comment optional, displayed in the case of failure @param {Boolean} value that is expected to be true @throws GsUnit.Failure if the given value is not true @throws GsUnitInvalidAssertionArgument if the given value is not a boolean or if an incorrect number of arguments is passed */
function assert()
{
...
/* Checks that a boolean value is false. @param {String} comment optional, displayed in the case of failure @param {Boolean} value that is expected to be false @throws GsUnit.Failure if value is not false @throws GsUnitInvalidAssertionArgument if the given value is not a boolean or if an incorrect number of arguments is passed */
...
/* Checks that two values are not equal (using !==) @param {String} comment optional, displayed in the case of failure @param {Value} value1 a value @param {Value} value2 another value @throws GsUnit.Failure if the values are equal @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that a value is null @param {String} comment optional, displayed in the case of failure @param {Null} value the value @throws GsUnit.Failure if the value is not null @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
function assertNull()
{
	GsUnit.validateArguments(1, arguments);
...
/* Checks that a value is not null @param {String} comment optional, displayed in the case of failure @param {Value} value the value @throws GsUnit.Failure if the value is null @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that a value is undefined @param {String} comment optional, displayed in the case of failure @param {Value} value the value @throws GsUnit.Failure if the value is not undefined @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that a value is not undefined @param {String} comment optional, displayed in the case of failure @param {Value} value the value @throws GsUnit.Failure if the value is undefined @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that a value is NaN (Not a Number) @param {String} comment optional, displayed in the case of failure @param {Value} value the value @throws GsUnit.Failure if the value is a number @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
function assertNaN()
{
	GsUnit.validateArguments(1, arguments);
	var aVar = GsUnit.nonCommentArg(1, 1, arguments);
...
/* Checks that a value is not NaN (i.e. is a number) @param {String} comment optional, displayed in the case of failure @param {Number} value the value @throws GsUnit. Failure if the value is not a number @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that an object is equal to another using === for primitives and their object counterparts but also desceding into collections and calling assertObjectEquals for each element @param {String} comment optional, displayed in the case of failure @param {Object} value the expected value @param {Object} value the actual value @throws GsUnit.Failure if the actual value does not equal the expected value @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
function assertObjectEquals()
{
	...
	{
		var primitiveEqualityPredicate = GsUnit.PRIMITIVE_EQUALITY_PREDICATEStypeOfVar1;
...
/* Checks that an array is equal to another by checking that both are arrays and then comparing their elements using assertObjectEquals @param {String} comment optional, displayed in the case of failure @param {Array} value the expected array @param {Array} value the actual array @throws GsUnit.Failure if the actual value does not equal the expected value @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that a value evaluates to true in the sense that value == true @param {String} comment optional, displayed in the case of failure @param {Value} value the value @throws GsUnit.Failure if the actual value does not evaluate to true @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
function assertEvaluatesToTrue()
{
	GsUnit.validateArguments(1, arguments);
	var value = GsUnit.nonCommentArg(1, 1, arguments);
	if (!value) fail(GsUnit.commentArg(1, arguments));
}

/* Checks that a value evaluates to false in the sense that value == false @param {String} comment optional, displayed in the case of failure @param {Value} value the value @throws GsUnit.Failure if the actual value does not evaluate to true @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that a hash is has the same contents as another by iterating over the expected hash and checking that each key's value is present in the actual hash and calling assertEquals on the two values, and then checking that there is no key in the actual hash that isn't present in the expected hash. @param {String} comment optional, displayed in the case of failure @param {Object} value the expected hash @param {Object} value the actual hash @throws GsUnit.Failure if the actual hash does not evaluate to true @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
function assertHashEquals()
{
	...
	for (var key in var1)
	{
		assertNotUndefined("Expected hash had key " + key + " that was not found", var2key);
		assertEquals("Value for key " + key + " mismatch - expected = " + var1key + ", actual = " + var2key, var1key, var2key);
	}
	for (var key in var2)
	{
		assertNotUndefined("Actual hash had key " + key + " that was not expected", var1key);
	}
}

/* Checks that two value are within a tolerance of one another @param {String} comment optional, displayed in the case of failure @param {Number} value1 a value @param {Number} value1 another value @param {Number} tolerance the tolerance @throws GsUnit. Failure if the two values are not within tolerance of each other @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments is passed */
...
/* Checks that a collection contains a value by checking that collection.indexOf(value) is not -1 @param {String} comment optional, displayed in the case of failure @param {Value} collection the collection @param {Value} value the value @throws GsUnit.Failure if the collection does not contain the value @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments are passed */
...
/* Checks that two arrays have the same contents, ignoring the order of the contents @param {String} comment optional, displayed in the case of failure @param {Array} array1 first array @param {Array} array2 second array @throws GsUnit.Failure if the two arrays contain different contents @throws GsUnitInvalidAssertionArgument if an incorrect number of arguments are passed */
function assertArrayEqualsIgnoringOrder()
{
	...
	for (var i = 0; i < var1.length; i++)
	{
		...
		for (var j = 0; j < var2.length; j++)
		{
			try
			{
				assertObjectEquals(notEqualsMessage, var1i, var2j);
				found = true;
			}
			catch (ignored)
			{}
...
/* Causes a failure @param failureMessage the message for the failure */
function fail(failureMessage)
{
	throw new GsUnit.Failure("Call to fail()", failureMessage);
}

/* @class A GsUnit.Failure represents an assertion failure (or a call to fail()) during the execution of a Test Function @param comment an optional comment about the failure @param message the reason for the failure */
GsUnit.Failure = function (comment, message)
{
	...
	var failComment = '';
	if (comment != null)
		failComment = 'Comment: ' + comment;
	
	throw failComment + ' -- Failure: ' + message;
}

/* @class A GsUnitAssertionArgumentError represents an invalid call to an assertion function - either an invalid argument type or an incorrect number of arguments @param description a description of the argument error */
...
/* @class @constructor Contains utility functions for the GsUnit framework */
GsUnit.Util = {};

/* Standardizes an HTML string by temporarily creating a DIV, setting its innerHTML to the string, and the asking for the innerHTML back @param html */
...
/* Returns whether the given string is blank after being trimmed of whitespace @param string */
GsUnit.Util.isBlank = function (string)
{
	return GsUnit.Util.trim(string) == '';
}

/* Implemented here because the JavaScript Array.push(anObject) and Array.pop() functions are not available in IE 5.0 @param anArray the array onto which to push @param anObject the object to push onto the array */
GsUnit.Util.push = function (anArray, anObject)
{
	anArray[anArray.length] = anObject;
}

/* Implemented here because the JavaScript Array.push(anObject) and Array.pop() functions are not available in IE 5.0 @param anArray the array from which to pop */
...
/* Returns the name of the given function, or 'anonymous' if it has no name @param aFunction */
GsUnit.Util.getFunctionName = function (aFunction)
{
	var regexpResult = aFunction.toString().match(/function(\s)(\w)/);
	
	if (regexpResult && regexpResult.length >= 2 && regexpResult2)
	{
		return regexpResult2;
	}
...
/* Returns an array of stack trace elements from the given exception @param exception */
...
/* Strips whitespace from either end of the given string @param string */
